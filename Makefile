...
routers:
	...
	@i=$(DEIS_FIRST_ROUTER) ; while [ $$i -le $(DEIS_NUM_ROUTERS) ] ; do \
			cp router/systemd/deis-router.service ./deis-router.$$i.service ; \
			fleetctl --strict-host-key-checking=false submit ./deis-router.$$i.service ; \
			fleetctl --strict-host-key-checking=false start ./deis-router.$$i.service ; \
			rm -f ./deis-router.$$i.service ; \
			i=`expr $$i + 1` ; \
	done
